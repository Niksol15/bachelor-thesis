...
перебувають у стані активного розвитку. Так, наприклад, стаття @EdDSA в якій була запропонований одна з найбільш популярних зараз еліптичних
...
Під $ZZ_p^*$ мається увазі кільце лишків за модулем p, де p - просте число.
...
вживають термін ECDLP), аналогічно до складно обчислювальної проблеми в Diffie-Hellman і ElGamal. До того ж, еліптичні криві мають додаткову
...
+ Імплементація додавання точок еліптичної кривої має працювати за константний час, інакше можлива timing attack що викриває приватний ключ. Як можна побачити,
...
У 2015 для кривих Веєрштрасса знайшли формулу для complete addition law @WCAL. Але, оскільки формула дуже об'ємна, вона тут не наводиться.
...
== Pairing(Спарювання точок еліптичної кривої)
...
Далі, почалося дослідження цієї кривої над $FF_(2^m)$, для цього поля вона має вигляд:
...
- Алгоритми $rho$ і $lambda$ Полларда. $rho$ є узагальненням $lambda$ алгоритму. Вони також мають часову складність $O(sqrt(q))$, але є потребують тільки $O(1)$ додаткового місця. Також треба зазначити що цей алгоритм є ймовірніснім, тобто він знаходить дискретний логарифм з якиюсь високою не 100% ймовірністю, якщо логарифм існує.
- The Pohlig-Hellman method @PohligHellman(Метод Поліга-Геллмана). Ефективний якщо $q$ не є простим і в його факторизації нема великих простих чисел. Нехай $q = product_i p_i ^ e_i$, тоді складність часова складність цього алгоритму $O(sum_i e_i(log q  + sqrt(p_i)))$
...
- The Frey-Rück Attack @FrayRuck (Атака Фрея-Рюка). Аналогічно MOV, тільки використовується більш ефективний Tate pairing.
...
Тепер розглянемо який чином кодуються/декодуються точки еліптичної кривої, найрозповсюдженішим стандартом є SEC1 @SEC1. Нехай у нас є закодована точка $P in E(FF_p)$ в послідовність байтів:
- Якщо перший байт 0х00 - це означає що $P = cal(O)$.
- Інакше, якщо перший байт 0х02 або 0х03 - це означає закодована у стисненому форматі (compressed form) (0х02|0x03)||$x$, де $x$ - це $x(P)$. Знаючи $x$ можна обчислити $y^2$ і знайти з нього 2 кореня. 0x02 означає що треба взяти парний корінь, 0х03 - непарний.
...
Розглянемо одні з найрозповсюженіших кривих що використовуються у сучасній криптографії що надають 128 бітний рівень безпеки.

=== secp256r1 (P256)

Це крива була затверджена NIST в стандарті SEC2 @SEC2 у 1999 році. Це крива є єдиною обов'язковою кривою TLS 1.3. r у назві ознчає random(випадкова). secp256r1 визанчена наступним чином:
- Крива визначена над $p_r = 2^256 − 2^224 + 2^192 + 2^96 − 1$. Така структура $p_r$ призначена пришвидшити арифметику по модулю $p_r$
- Крива має рівняння у формі Веєрштрасса $y^2 = x^3 - 3x + b,$ де $b in FF_(p_r)$ можна записати у шістнадцятеричному вигляді як $b = "5ac635d8 aa3a93e7 b3ebbd55 769886bc 651d06b0 cc53b0f6 3bce3c3e 27d2604b"$
- $|E(FF_p)|$ - просте число
...
Ця крива також була затверджена в SEC2. Вона шикоро використовуються в блокчейнах, наприклад, ця крива крива використовуюється в блокчейнах Bitcoin і Ethereum. secp256r1 визанчена наступним чином:
...
Як можна зазничи у цієї кривої немає випадкового парметра.
...
Розглянемо протоколи узгодження ключів що використовують еліптичні криві.

=== ECDH

ECDH(Elliptic curve Diffie-Hellman) - протокол Діффі-Геллмана для еліптичних кривих. Цей протокол використовується для узгодження спільного секрету, на практиці wим спільним секретом є симетричний ключ. Розглянемо сам протокол.
...
Як можна побчити з (@dh-cmp) стандарти, що використовують еліптичні криві в протоколі Діффі-Геллмана працють набагато швидше стандартів що використовують $ZZ_q$. Цей бенчмарк показав прирост швидкії у 6 разів. Варто відзначити швидкодію X25519, що побудований на Curve25519. 
...
Це підпис який реалізований за допомогою спарбвання точок еліптичної кривої. Тут не наводиться докладний опис, з ним можна ознайомитися в @BonehShoup.
...
Цільовим рівнем безпеки для імплементованої бібліотеки є 256 бітний ключ ECC, який відповідає рівню безпеки симетричного шифрування з 128 бітним ключем. Як було визначено раніше, Сurve25519 має кращі показники швидкодії порівнюючи з P256 та secp256k1. Також, деякі нові блокчейни(наприклад @Monero) стартують використовувати Сurve25519, замість secp256k1, тож можна припустити що бенчмарки покзали правильний результат і Сurve25519 дійсно має найкращу швидкодію серед описаних у цій роботі кривих. Тому бібліотека імплементує криптосистеми саме на Сurve25519.

Для розробки бібліотеки яка імплементує криптосистеми з використанням еліптичних кривих використаний 20 стандарт мови програмування С++. Імплементація бібліотеки, тести і наведені вище бенчмарки знaходиться за посиланням @ECCPP.

=== Curve25519

Коректа, швидка і безпечна імплементація самої кривої, а також елементів $FF_p$, дуже складна і часозатратна задача, оскільки неправильна імплементація може призвести до того, що криптосистема на її основі буде вразлива до атак. Тому було вирішено взяти імплементації елементів $FF_p$ з libsodium.

=== Імплементація X25519
=== Імплементація Ed25519
