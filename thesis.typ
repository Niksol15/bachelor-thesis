...
Еліпітичні криві є не просто об'єктом вивчення аналітичної геометрії, а й ціким об'єктом алгебри, бо над точками еліптичної кривої можна проводити обчислення. Перед переходом до еліптичних кривих над скінченими полями, для більш інтуітивного розуміння, розглянемо як "додавати" точки
еліптичної кривої над $RR$, бо це можна легко зобразити графічно. Позначемо операцію додавання точок еліптичної кривої як $plus.circle$ і розглянемо її на прикладі з @MathCrypto. Для початку визначимо, що якщо $A = (x, y) in E text(", то") (-A) = (x, -y) in E$. Нехай $E slash RR$ - це еліптична крива задана рівнянням $y^2 = x^3 - 15x + 18$; Точки $P = (7, 16), Q = (1, 2)$ належать $E$, тоді пряма $L$ що їх сполучає задається рівнянням: $y = 7/3 x - 1/3$. Для того щоб знайти в яких точках $L$ перетинає $E$ ми можемо підставити заміcть $y$ рівність з $L$ в $E$ і розв'язати рівняння відносно $x$. Для наведеного прикладу маємо:

$(7/3 x - 1/3) ^ 2 = x^3 -15 x + 18 arrow.r x^3 - 49/9 x^2 - 121/9 x + 161/9 = 0$

Отримане рівняння є рівнянням 3 степені, отже воно має 3 корені, 2 з яких нам вже відомі, залишається знайти 3 корінь. Найлегшим способом є поділити отриманий поліном на $(x - 7)(x - 1)$. Таким чином отримуємо 3 корінь $x = -23/9$. Підставляючи $x$ в $E$ отримуємо $y = -170/27$, а отже точку $R = (-23/9, -170/27)$. Далі відображаємо $R$ відносно OX і маємо 
...
Залишиось розглянути останній випадок: $P plus.circle (-P)$. Для цього введемо спеціальну точку, що називається точкою на нескінченості: $cal(O)$, тоді $P plus.circle (-P) = cal(O)$. Графічно це виглядає наступним чином:
...
Нехай $p > 3$ - простe число. Тоді еліптичною кривою $E$ визначеною над $FF_p$ є множина розв'язків рівняння $y^2 = x^3 + a*x + b$, де $a, b in FF_p$ задовільняють умову $4 a^3 + 27 b^2 eq.not 0$. Ця умова потрібна щоб впевнитись, що рівння $x^3 + a x + b = 0$ має тільки один корінь. У випадку, якщо попередня властивість не виконується, рівняння кривої буде мати 2, або 3 однакових кореня. Нехай у такому випадку коренем рівняння кривої буде $x_0$, тоді точка $(x_0, 0)$ називається сингулярною і відповідна крива теж називається сингулярною.
...
Тепер розглянемо $plus.circle$ для $E(FF_p)$. Для цього будемо спиратися на результати розгляду $(E(RR), plus.circle)$, бо для $E(FF_p)$ алгоритм виглядає аналогічно до алгоритму для $E(RR)$, але його не вдасться легко інтерпретувати геометрично. Розглядом геометричної інтерпретації обчислень над точками еліптичних кривих над скінченими полями займається алгебраїчна геометрія і це виходить за межі даної роботи. Нехай $P = (x_p, y_p) or cal(O), Q = (x_q, y_q) or cal(O) in E(FF_p)$:
...
+ Імплементація додавання точок еліптичної кривої має працювати за константний час, інакше можлива атака що викриває приватний ключ. Яе можна побачити даний алгоритм важко реалізувати таким чином, щоб він працював за константиний час через різну кількість обчисленнь за різних умов.

== Характеристики і особливості еліптичних кривих
=== $|E(FF_p)|$
...
складності роботи цього алгоритму @SEA. Результат називається SEA, він залишається найшвидшим алгоритмом до цього часу і має складність  $O(log^2 p * M(log^2 p) slash log log p)$, де $M(n)$ - складність множення.
...
Очевидний приклад: $3P = P + 2P = P + P + P$. Також зрозуміло що обчислювати $n P$ наївно, за $n - 1$ додавань - неоптимально, тому часто використовують оптимізацію, яка називаєтсья double-and-add(подвоюй-і-додавай). З її допомогою можна обчислити $n P$ за не більше ніж $2 log_2 n$ додавань. Існують 2 версії цього алгоритму: яка використовує інверсію($P -> -P$) і яка не використовує.
Оскільки знаходження точки еліптичної кривої оберненою до даної - тривільне і швидке, і перша версія працює за не більшу кількість додавань ніж 2, то використовують частіше її. Я не наводжу тут алгоритм, оскільки я наведу в наступному розділі його реалізацію.

Швидкість обчислення $n P$ є важливою характеристикою кривої. Існують криві, для яких можна зоптимізувати як одиничну операцію $P + Q$, так і обчислення $n P$ в цілому.

== Припущення на яких будується ECC

У цьому підрозілі розглядаються припущення, на які опираються ECC криптосистеми.

=== ECDLP
Почнемо з формального визначення:

Нехай $E$ - еліптична крива, $FF_p$ - скінчене поле, $P in E(FF_p), Q in angle.l P angle.r$, тоді дискретним логарифмом $Q$ за основою $P$ є таке $n$, що $Q = n P$ і позначається $n = log_P Q$.

Нехай $q$ - порядок $P$, тоді очевидно що, якщо $Q = n_0 P$, то $Q = (n_0  + i q) P, forall i in NN$. Для визначеності будемо вважати, що дискретний логарифм - це найменше таке $n$, що $Q = n P$. У такому випадку зрозуміло, що $log_P(Q) in Z_q$.
...
$log_P: angle.l P angle.r arrow.r Z_q$

ECDLP - це проблема знаходження дискретного логарифму для еліптичної кривої. Припущення полягає у тому, що ECDLP для точок еліптичної кривої над скінченим полем є складно обчислювальною проблемою, тобто не існує ефективного алгоритму обчислення дискретного логарифму.

На ECDLP існує багато атак - ефективних алгоритмів розв'язання. Ним присвячена ціла глава в @ECNTC. Перша група атак працює для будь якої циклічної групи:
-
-
-

Такж існує група атак, що використовують додаткову структуру кривих - pairing(спарюваня). Визначення і побудова pairing будуть описані згодом. Такими атаками є:

-
-
-

Оскільки ECC грунтується на припущені про обчислювальну складність ECDLP і ECC активно використовується в реальних криптосистемах можна припустити, що для будь якої кривої $E/FF_p$, де $p$ - достатньо велике число ECLP складнообчислювальне, але це є хибним твердженням, у тому числі через наведені до цього атаки, наприклад:

- Якщо $|E(FF_p)|$ не є простим і всі прості множники у його розкладі менші деякого $q_max$, то тоді існує алгоритм обчисленя дискретного логарифму в $|E(FF_p)|$ за час $Theta(sqrt(q_max))$. Зокрема, якщо $q_max lt 2 ^ 80$, то ECDLP не є складнообчислювальною проблемою на практиці. Тому в реальних системах використовують криві $E slash FF_p$ для яких $|E(FF_p)|$ рівне $q, 4q, text("або") 8q$ для деякого простого $q$.  
...
- Нехай існує таке мале $tau > 0 in NN$, що $|E(FF_p)|$ ділить $p^tau - 1$. Тоді ECDLP в $E(FF_p)$ можна звести до DLP в $FF_(p^tau)$, що в свою чергу дозволить використати GNFS. На практиці, якщо $p$ - 256 бітне число і $tau = 2$, то розв'язок ECDLP займе приблизно пару годин. Для захисту від цієї атаки, треба переконатися що $p^tau$ достатньо велике для того щоб GNFS в $FF_(p ^ tau)$ був нездійсненний.

Але, при виконані деякої множини вимог до еліптичної кривої, найоптимальніший алгоритм вирішення ECLP працює не швидше ніж найоптимальніший алгоритм вирішення DLP для довільної циклічної групи. Тобто, якщо параметри кривої підібрано правильно, то ECDLP має часову складність $O(sqrt(q)) text(", де") q = |E(FF_p)|$.

=== CDH
=== DDH

З цього розділу має бути очевидно, що розробка своєї еліптичної кривої - це складна справа, до того ж, неправильно підібрана еліптична крива може призвести до того, що криптосистеми з її використанням не будуть безпечними.
Тому на практиці використовують набір затверджених, або просто перевірених кривих. Деякі з них будуть розглянуті у цьому розділі далі. 
