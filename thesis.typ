...
Далі відображаємо $R$ відносно OX і маємо 
...
З її допомогою можна обчислити $n P$ за не більше ніж $2 log_2 n$ додавань. Існують 2 версії цього алгоритму: яка використовує інверсію($P -> -P$) і яка не використовує. Оскільки знаходження точки еліптичної кривої оберненою до даної - тривільне і швидке, і перша версія працює за не більшу кількість додавань ніж друга, то використовують частіше її. Я не наводжу тут алгоритм, оскільки я наведу в наступному розділі його реалізацію.
...
для $ZZ_p^*$ і дозволяє розширити множину криптосистем ECC. Зазвичай у визначенні використовують мультиплікативні групи, замість адитивних, але оскільки ми концентруємося
...
Коли $GG_0 = GG_1$ ми кажемо що спарювання симетричне, інакше - асиметричне.
...
Вимога недегенеративності потрібна щоб $e$ не повертала завжди $cal(O) in GG_T$ для всіх аргументів.
...
Криві Монгомері дозволяють пришвидшити операцію додавання, шляхом оптимізації Montgomery ladder.
...
Аналогічно кривій Монтгомері, крива Едвардса може бути приведена до форми Вейерштраса заміною зміних, але не навпаки. $|E(FF_p)|$ завжди ділиться на 4.
...
ECDLP - це проблема знаходження дискретного логарифму для еліптичної кривої. Припущення полягає у тому, що ECDLP для точок еліптичної кривої над скінченим полем є 
...
кривої $E slash FF_p$, де $p$ - достатньо велике число ECLP складнообчислювальне, але це є хибним твердженням, у тому числі через наведені до цього атаки, наприклад:

- Якщо $|E(FF_p)|$ не є простим і всі прості множники у його розкладі менші деякого $q_max$, то тоді існує алгоритм обчисленя дискретного логарифму в $|E(FF_p)|$ за час $Theta(sqrt(q_max))$. 
Зокрема, якщо $q_max lt 2 ^ 80$, то ECDLP не є складнообчислювальною проблемою на практиці. Тому в реальних системах використовують криві $E slash FF_p$ 
для яких $|E(FF_p)|$ рівне $q, 4q, text("або") 8q$ для деякого простого $q$.  
...
- Нехай існує таке мале $tau > 0 in NN$, що $|E(FF_p)|$ ділить $p^tau - 1$. Тоді ECDLP в $E(FF_p)$ можна звести до DLP в $FF_(p^tau)$, що в свою чергу дозволить використати GNFS.
На практиці, якщо $p$ - 256 бітне число і $tau = 2$, то розв'язок ECDLP займе приблизно пару годин. Для захисту від цієї атаки, треба переконатися що $p^tau$ достатньо велике 
для того щоб GNFS в $FF_(p ^ tau)$ був нездійсненний.

Але, при виконані деякої множини вимог до еліптичної кривої, найоптимальніший алгоритм вирішення ECLP працює не швидше ніж найоптимальніший алгоритм вирішення DLP для 
довільної циклічної групи. Тобто, якщо параметри кривої підібрано правильно, то ECDLP має часову складність $O(sqrt(q)) text(", де") q = |E(FF_p)|$.

=== CDH
=== DDH

З цього розділу має бути очевидно, що розробка своєї еліптичної кривої - це складна справа, до того ж, неправильно підібрана еліптична крива може призвести до того,
що криптосистеми з її використанням не будуть безпечними. Тому на практиці використовують набір затверджених, або просто перевірених кривих. Деякі з них будуть розглянуті у цьому розділі далі. 
