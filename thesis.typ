...
- Нехай існує таке мале $tau > 0 in NN$, що $|E(FF_p)|$ ділить $p^tau - 1$. Тоді ECDLP в $E(FF_p)$ можна звести до DLP в $FF_(p^tau)$ використовуючи MOV, атаку Фрея-Рюка, що в свою чергу дозволить використати GNFS. На практиці, якщо $p$ - 256 бітне число і $tau = 2$, то розв'язок ECDLP займе приблизно пару годин. Для захисту від цієї атаки, треба переконатися що $p^tau$ достатньо велике для того щоб GNFS в $FF_(p ^ tau)$ був нездійсненний.
